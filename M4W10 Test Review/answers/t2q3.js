...
- range(1, false, <anything>) should return [0]
...
const range = function(count, skipZero, descending) {
  ...
  if (skipZero === true && descending === true) {
    x = counter - 1;
    for (x; x >= 1; x--) {
      finalArray.push(x);
    }
  ...
  // if(descending) {
  //   for(let i = count + lowestNumber -1 ; i >= lowestNumber; i-- ) {
  //     result.push(i)
  //   }  
  // }  else {
  //   for (let i = lowestNumber; i <= count + lowestNumber - 1 ; i ++) {
  //     result.push(i)
  //   }
  // }
